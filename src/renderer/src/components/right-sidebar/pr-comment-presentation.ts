...
export type PRCommentPresentationClasses = {
  ...
  /** False where the surface already names the file, so the badge carries only the lines. */
  pathBadgeShowsFile: boolean
  list: string
  group: string
...
export function getPRCommentPresentationClasses(
  ...
  return {
    ...
    commentRowReply: `border-t ${COMMENT_CARD_DIVIDER}`,
    ...
    // the far edge and read as belonging to something else. The spacer after them still pushes the
    // row actions right.
    author: `min-w-0 truncate ${CARD_COMMENT_AUTHOR_SIZE} font-semibold text-foreground`,
    authorResolved: 'text-muted-foreground',
    avatar: `size-5 self-center ${COMMENT_AVATAR}`,
    avatarReply: `size-4 self-center ${COMMENT_AVATAR}`,
    botBadge:
      'shrink-0 rounded border border-border bg-muted px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground',
    ...
    // Why no indent or rule: a reply is already marked by its own top border and tinted row, so
    // stepping it in as well only narrows the text it is trying to show.
    ...
    commentHeaderMeta: cn(
      ...
      'flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1 text-[11px] text-muted-foreground'
    ),
